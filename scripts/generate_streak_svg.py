...
USER = sys.argv[1] if len(sys.argv) > 1 else "Bilmpz"
...
for wk in range(NW):
    ...
    if d.month != last_m:
        last_m = d.month
        mlabels.append((wk, MONTHS[d.month-1]))
if len(mlabels) > 1 and mlabels[1][0] - mlabels[0][0] < 3:
    mlabels = mlabels[1:]   # første delmåned kolliderer med næste label
for wk, name in mlabels:
    labels.append(f'<text class="lbl" x="{LEFT+wk*(CELL+GAP)}" y="{TOP-8}">{name}</text>')
...
<style>
  ...
  @media (prefers-color-scheme: light) {{ text.total {{ fill:#1f2328; }} text.lbl {{ fill:#57606a; }} rect.e {{ fill:#ebedf0; }} }}
